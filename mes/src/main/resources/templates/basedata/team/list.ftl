...
<script type="text/html" id="js-record-table-toolbar-right">
    <a class="layui-btn layui-btn-xs layui-btn-warm" lay-event="teamUser"><i class="layui-icon layui-icon-user"></i>员工</a>
...
<script>
    layui.use(['form', 'table', 'spLayer', 'spTable'], function () {
        ...
        table.on('toolbar(js-record-table-filter)', function (obj) {
            if (obj.event === 'deleteBatch') {
                ...
                if (data.length > 0) {
                    layer.confirm('确认要删除选中的 ' + data.length + ' 条记录吗？', function (index) {
                        ...
                        spUtil.ajax({
                            url: '${request.contextPath}/basedata/team/batch-delete',
            ...
            if (obj.event === 'add') {
                spLayer.open({ title: '添加', area: ['800px', '450px'], content: '${request.contextPath}/basedata/team/add-or-update-ui' });
            }
        });

        table.on('tool(js-record-table-filter)', function (obj) {
            var data = obj.data;
            if (obj.event === 'teamUser') {
                spLayer.open({ title: '员工分配 - ' + data.name, area: ['600px', '500px'], spWhere: {teamId: data.id}, content: '${request.contextPath}/basedata/team/team-user-ui' });
            }
            if (obj.event === 'edit') {
                spLayer.open({ title: '编辑', area: ['800px', '450px'], spWhere: {id: data.id}, content: '${request.contextPath}/basedata/team/add-or-update-ui' });
            }
            if (obj.event === 'delete') {
                layer.confirm('确认要删除吗？', function (index) {
                    spUtil.ajax({
                        url: '${request.contextPath}/basedata/team/delete', type: 'POST', serializable: false,
                        data: {id: data.id},
                        success: function () { tableIns.reload(); layer.close(index); }
                    });
                });
