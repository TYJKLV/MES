...
<script type="text/html" id="js-record-table-toolbar-right">
    <a class="layui-btn layui-btn-xs layui-btn-warm" lay-event="flowBom"><i class="layui-icon layui-icon-link"></i>BOM绑定</a>
    {{# if(d.state == 'creat'){ }}
...
<script>
    layui.use(['form', 'table', 'spLayer', 'spTable'], function () {
        ...
        var tableIns = spTable.render({
            url: '${request.contextPath}/technology/flow/page',
            cols: [[
                {type: 'checkbox'},
        ...
        table.on('toolbar(js-record-table-filter)', function (obj) {
            if (obj.event === 'deleteBatch') {
                ...
                if (data.length > 0) {
                    layer.confirm('确认要删除选中的 ' + data.length + ' 条记录吗？', function (index) {
                        ...
                        spUtil.ajax({
                            url: '${request.contextPath}/technology/flow/batch-delete',
            ...
            if (obj.event === 'add') {
                spLayer.open({ title: '添加', area: ['800px', '500px'], content: '${request.contextPath}/technology/flow/add-or-update-ui' });
            }
        });

        table.on('tool(js-record-table-filter)', function (obj) {
            var data = obj.data;
            if (obj.event === 'flowBom') {
                spLayer.open({ title: 'BOM绑定 - ' + data.flow, area: ['600px', '500px'], spWhere: {flowId: data.id}, content: '${request.contextPath}/technology/flow/bom-ui' });
            }
            if (obj.event === 'lock') {
                layer.confirm('确认定版此工艺路线？定版后不可编辑', function (index) {
                    spUtil.ajax({ url: '${request.contextPath}/technology/flow/lock', type: 'POST', serializable: false, data: {id: data.id}, success: function () { tableIns.reload(); layer.close(index); } });
                });
            }
            if (obj.event === 'unlock') {
                layer.confirm('确认取消定版？', function (index) {
                    spUtil.ajax({ url: '${request.contextPath}/technology/flow/unlock', type: 'POST', serializable: false, data: {id: data.id}, success: function () { tableIns.reload(); layer.close(index); } });
                });
            }
            if (obj.event === 'edit') {
                spLayer.open({ title: '编辑', area: ['800px', '500px'], spWhere: {id: data.id}, content: '${request.contextPath}/technology/flow/add-or-update-ui' });
            }
            if (obj.event === 'delete') {
                layer.confirm('确认要删除吗？', function (index) {
                    spUtil.ajax({ url: '${request.contextPath}/technology/flow/delete', type: 'POST', serializable: false, data: {id: data.id}, success: function () { tableIns.reload(); layer.close(index); } });
                });
            }
        });
    });
