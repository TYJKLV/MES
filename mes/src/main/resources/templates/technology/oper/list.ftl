...
<script>
    layui.use(['form', 'table', 'spLayer', 'spTable'], function () {
        ...
        var tableIns = spTable.render({
            ...
            cols: [[
                {type: 'checkbox'},
                {field: 'operCode', title: '工序编号'},
                {field: 'oper', title: '工序名称'},
                {field: 'operDesc', title: '工序描述'},
                {field: 'manHours', title: '工时定额(分钟)'},
                {field: 'workcellId', title: '加工单元ID'},
        ...
        table.on('toolbar(js-record-table-filter)', function (obj) {
            if (obj.event === 'deleteBatch') {
                ...
                if (data.length > 0) {
                    layer.confirm('确认要删除选中的 ' + data.length + ' 条记录吗？', function (index) {
                        ...
                        spUtil.ajax({
                            url: '${request.contextPath}/technology/oper/batch-delete',
            ...
            if (obj.event === 'add') {
                spLayer.open({ title: '添加', area: ['800px', '480px'], content: '${request.contextPath}/technology/oper/add-or-update-ui' });
            }
        });
        ...
        table.on('tool(js-record-table-filter)', function (obj) {
            var data = obj.data;
            if (obj.event === 'edit') {
                spLayer.open({ title: '编辑', area: ['800px', '480px'], spWhere: {id: data.id}, content: '${request.contextPath}/technology/oper/add-or-update-ui' });
            }
            if (obj.event === 'delete') {
                layer.confirm('确认要删除吗？', function (index) {
                    spUtil.ajax({
                        url: '${request.contextPath}/technology/oper/delete', type: 'POST', serializable: false,
                        data: {id: data.id},
                        success: function () { tableIns.reload(); layer.close(index); }
                    });
                });
