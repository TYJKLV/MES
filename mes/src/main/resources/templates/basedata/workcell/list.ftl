...
<script>
    layui.use(['form', 'table', 'spLayer', 'spTable'], function () {
        ...
        var tableIns = spTable.render({
            url: '${request.contextPath}/basedata/workcell/page',
            cols: [[
                {type: 'checkbox'},
                {field: 'code', title: '单元编码'},
                {field: 'name', title: '单元名称'},
                {field: 'deptId', title: '所属车间ID'},
                {field: 'groupId', title: '设备编组ID'},
        ...
        table.on('toolbar(js-record-table-filter)', function (obj) {
            if (obj.event === 'deleteBatch') {
                ...
                if (data.length > 0) {
                    layer.confirm('确认要删除选中的 ' + data.length + ' 条记录吗？', function (index) {
                        ...
                        spUtil.ajax({
                            url: '${request.contextPath}/basedata/workcell/batch-delete',
            ...
            if (obj.event === 'add') {
                spLayer.open({ title: '添加', area: ['800px', '450px'], content: '${request.contextPath}/basedata/workcell/add-or-update-ui' });
            }
        });
        ...
        table.on('tool(js-record-table-filter)', function (obj) {
            var data = obj.data;
            if (obj.event === 'edit') {
                spLayer.open({ title: '编辑', area: ['800px', '450px'], spWhere: {id: data.id}, content: '${request.contextPath}/basedata/workcell/add-or-update-ui' });
            }
            if (obj.event === 'delete') {
                layer.confirm('确认要删除吗？', function (index) {
                    spUtil.ajax({
                        url: '${request.contextPath}/basedata/workcell/delete', type: 'POST', serializable: false,
                        data: {id: data.id},
                        success: function () { tableIns.reload(); layer.close(index); }
                    });
                });
